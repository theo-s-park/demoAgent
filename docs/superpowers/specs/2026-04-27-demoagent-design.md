...
**AWS 예시:** **VPC** 안에 **EC2**(Docker Compose로 Spring·FastAPI 도구 컨테이너)와 **RDS for MySQL**(프라이빗 서브넷)을 둔다. 브라우저는 EC2(또는 ALB) **:8080**으로 접속하고, Agent는 Internet의 **OpenAI API**를 HTTPS로 호출한다. **RDS에는** 도구 메타·헬스 이력, `system_prompt` 버전, Agent 세션/실행 로그, Tool Creator 잡 등 **영속 데이터**를 두고, **EC2 볼륨(또는 mount)** 에는 `tool-server/*.py`·`system-prompt.txt` 등 **런타임 파일**을 둔다(역할 분리). 현재 데모가 파일 위주면, 운영에서는 이 스키마를 MySQL로 옮기면 된다.

**Excalidraw 다이어그램:** `docs/superpowers/architecture-demoagent.excalidraw` — VPC·EC2(Docker)·**RDS(MySQL) 저장 항목**·컨테이너별 도구·동적 영역·①~⑥ 플로우(간격 넓힘). 재생성: `python scripts/build_architecture_excalidraw.py` (`icons-source`의 Simple Icons SVG base64 임베드).
...
participant State as Runtime State (system-prompt.txt)

User->>UI: 질문 입력
UI->>Agent: POST /api/agent/run (SSE)
Agent->>State: load system-prompt.txt (tool list)
...
### ② Add Tool — 도구 추가(키 입력 → 생성 → health gate → 영속화)
...
participant LLM as External LLM API
participant FS as Runtime Files (.env.local, tool-server/*.py, system-prompt.txt)
participant DB as SQLite tools.db (dynamic_tool)
participant Proc as uvicorn process (:8090+)

User->>UI: "도구 추가해줘" + (필요 시 API 키 입력)
UI->>Creator: POST /api/tool-creator/create (SSE)
Creator->>LLM: request (tool spec + answers)
LLM-->>Creator: response need_info OR create_tool
alt need_info
  Creator-->>UI: SSE form (questions)
  UI-->>User: 폼 렌더링
  User->>UI: answers 입력(API 키 등)
  UI->>Creator: POST /api/tool-creator/create (SSE, answers 포함)
  Creator->>LLM: request (answers 포함)
  LLM-->>Creator: response create_tool
end
...
Creator->>DB: insert/update dynamic_tool (tool_name, port, pid, created_at)
Creator-->>UI: SSE final (추가 완료)
UI-->>User: 새 도구 활성화 표시 + tool list refresh
```

### ③ Patch Prompt — 프롬프트 변경(패치/저장)
...
participant LLM as External LLM API
participant FS as Runtime Files (system-prompt.txt)

User->>UI: "프롬프트 변경" (지시/편집)
alt LLM patch (instruction 기반)
  UI->>Tools: POST /api/tools/prompt/patch
...
UI-->>User: tool list refresh (prompt 기반 목록 갱신)
```
